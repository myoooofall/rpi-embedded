...
void setup() {
  ...
  uint8_t DATA1[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
  ...
  pn532_error = PN532_MifareClassicWriteBlock(&pn532, DATA1, block_number_key);
  if (pn532_error) {
    Serial.print("Error: 0x");
    Serial.print(pn532_error, HEX);
    return;
  ...
  for (uint8_t i = 0; i < sizeof(DATA1); i++) {
    if (DATA1[i] != buff[i]) {
      ...
      Serial.print(block_number_key, DEC);
      Serial.print(" failed\r\n");
      return;
    }
  }
  Serial.print("Write the key of block ");
  Serial.print(block_number_key, DEC);
