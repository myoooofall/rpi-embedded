...
int read_imu_raw1(mraa::Uart* uart);
...
int read_imu_raw1(mraa::Uart* uart) {
    ...
    std::string buff_str = uart->readStr(IMU_DATA_LENGTH1);
    int length=buff_str.size();


    if(length > IMU_DATA_LENGTH1) length= IMU_DATA_LENGTH1;

    if(length==IMU_DATA_LENGTH1){
        std::cout<<"get status num: "<<IMU_DATA_LENGTH1 <<std::endl;
    }
    else if(length<IMU_DATA_LENGTH1){
        std::cout<<"get status num: "<<length<<std::endl;
    }
    std::copy(buff_str.begin(), buff_str.begin()+IMU_DATA_LENGTH1, buffer);
